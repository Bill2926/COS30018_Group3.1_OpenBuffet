# File: hybrid_test.py
# Evaluation & End-to-End Test module for HybridModel (VARIMAX + DL residual)
# Includes training pipeline and multi-target evaluation (Log-space & Price-space).
...
from models.dl_model import HybridModel, ModelFactory
...
if __name__ == "__main__":
    print("\n========================================================")
    print("      HYBRID VARIMAX + DL (TRAIN & EVALUATION FLOW)     ")
    print("========================================================\n")
    
    # 1. Configuration & Hyperparameters
    WINDOW = 10
    N_TARGETS = 2  # Multi-target: AAPL, QQQ
    N_EXOG = 2     # Exogenous features
    VARIMAX_ORDER = (1, 1)
    DL_TYPE = "lstm"
    EPOCHS = 3
    BATCH_SIZE = 16
    
    MODEL_PATH = os.path.join(MODELS_DIR, "HybridModel.keras")

    # 2. Generate Synthetic Time-Series Data (Simulation)
    np.random.seed(42)
    TOTAL_STEPS = 250
    TRAIN_LEN = 180

    # Simulate log returns
    y_all = np.random.randn(TOTAL_STEPS, N_TARGETS) * 0.01
    exog_all = np.random.randn(TOTAL_STEPS, N_EXOG)

    y_train, y_test = y_all[:TRAIN_LEN], y_all[TRAIN_LEN:]
    exog_train, exog_test = exog_all[:TRAIN_LEN], exog_all[TRAIN_LEN:]

    # Simulate raw price arrays for price reconstruction
    raw_close_aapl = 150.0 * np.exp(np.cumsum(y_all[:, 0]))
    raw_close_qqq = 380.0 * np.exp(np.cumsum(y_all[:, 1]))

    raw_close_map = {
        "AAPL": raw_close_aapl[TRAIN_LEN:],
        "QQQ": raw_close_qqq[TRAIN_LEN:]
    }

    # 3. Step 1: Train Hybrid Model
    print(">>> Step 1: Training Hybrid Model (VARIMAX + LSTM)...")
    hybrid_model = ModelFactory.get_model(
        model_type="hybrid",
    ...
    hybrid_model.build_model(num_layers=1, units=16, dropout_rate=0.1)
    
    # Train VARIMAX + Residual DL Sub-model & save weights + history
    hybrid_model.train(
        y_train=y_train,
    ...
    print(">>> Hybrid Model training completed & persisted to disk.\n")

    # 4. Step 2: Run Evaluation
    print(">>> Step 2: Running Evaluation via HybridValidation...")
    ...
    print("\n>>> End-to-end evaluation successfully finished!")